...
exports.CardiffController = void 0;
...
class CardiffController {
    constructor() {
        this.fireflyService = new firefly_service_1.FireFlyService(process.env.CARDIFF_FIREFLY_URL);
                        ...
                        if (messageData.type === 'STUDENT_BATCH_SUBMISSION_WITH_METADATA' && messageData.to === 'cardiff-met') {
                            ...
                            if (viewedFilter === null || messageViewedStatus === viewedFilter) {
                                inboxMessages.push({
                                    ...
                                    students: messageData.batch.students.map((student) => ({
    ...
    async processMessageAndIssueCertificates(req, res) {
        try {
            const { messageId } = req.body;
            ...
            const cardanoTxId = await this.cardanoService.submitCertificateHashesToCardano(batch.batch_id, batch.metadata.batch_name, certificateHashes, batch.metadata.academic_year, batch.metadata.semester, batch.metadata.faculty);
    ...
    // Add to Cardiff controller
    async getCertificatePDF(req, res) {
        try {
            const { certificateHash } = req.params;
            const student = await this.supabaseService.getStudentByHash(certificateHash);
            if (!student || !student.certificate_pdf) {
    ...
    async markMessageAsViewed(req, res) {
        try {
            const { messageId } = req.body;
    ...
    generateTransactionHash(student) {
        const studentData = `${student.student_id}-${student.first_name}-${student.last_name}-${Date.now()}`;
        return `tx_${(0, crypto_1.createHash)('sha256').update(studentData).digest('hex').substring(0, 16)}`;
    }
    async bulkVerifyStudents(req, res) {
        try {
    ...
    async processStudentVerification(verification) {
    }
    async getReceivedBatches(req, res) {
        try {
    ...
    async getBatchesWithMetadata(req, res) {
        try {
    ...
    async getBatchFullDetails(req, res) {
        try {
    ...
    async getStoredStudents(req, res) {
        try {
    ...
    async getCertifiedStudents(req, res) {
        try {
    ...
    async testOrganizations(req, res) {
        try {
...
exports.CardiffController = CardiffController;
