...
import { BatchMetadata, Student, StudentBatch } from '../models/student.model';

export class ICBTController {
  private fireflyService: FireFlyService;
  private supabaseService: SupabaseService;

  constructor() {
    this.fireflyService = new FireFlyService(process.env.ICBT_FIREFLY_URL!);
    this.supabaseService = new SupabaseService();
  }

  async submitStudentBatchWithMetadata(req: Request, res: Response): Promise<void> {
    try {
      console.log('📥 Received student batch submission with metadata from ICBT');
      const { metadata, students } = req.body as { metadata: BatchMetadata, students: Student[] };
      ...
      const requiredFields: (keyof BatchMetadata)[] = ['batch_name', 'academic_year', 'semester', 'graduation_ceremony_date', 'faculty', 'contact_person', 'contact_email'];
      ...
      const studentErrors: StudentError[] = [];
      students.forEach((student, index) => {
        const requiredStudentFields = ['student_id', 'first_name', 'last_name', 'email', 'course', 'graduation_date', 'gpa'];
        const missingStudentFields = requiredStudentFields.filter(field => !(student as any)[field]);
  ...
  async getSentBatches(req: Request, res: Response): Promise<void> {
    try {
  ...
  async testFireFlyConnection(req: Request, res: Response): Promise<void> {
    try {
...
async downloadStudentPDF(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (studentDetails && studentDetails.certificate_pdf) {
      ...
      res.setHeader('Content-Disposition', 
        `attachment; filename="certificate_${studentDetails.first_name}_${studentDetails.last_name}.pdf"`);
...
async getReceivedCertificates(req: Request, res: Response): Promise<void> {
  try {
          ...
          if (data && data.length > 0 && data[0].value) {
            ...
            if (batchInfo && batchInfo.students && batchInfo.students.length > 0) {
              ...
              submissionMap.set(batchInfo.batchId, {
                ...
                submittedStudents: batchInfo.students.map((s: any) => ({
        ...
        try {
          ...
          if (data && data.length > 0 && data[0].value) {
            ...
            if (messageData.type === 'CERTIFICATE_PDFS_ISSUED' && 
              ...
              const studentsWithCertificates = messageData.certificate_pdf_refs?.map((cert: any) => ({
                ...
                submitted: submissionInfo?.submittedStudents.some((s: any) => s.student_id === cert.student_id) || false
              })) || [];
    ...
    for (const [batchId, submission] of submissionMap) {
      if (!certificates.find(cert => cert.batchId === batchId)) {
        ...
        certificates.push({
          ...
          students: submission.submittedStudents.map((s: any) => ({
            ...s,
            certificate_id: null,
...
async debugSubmissionMessages(req: Request, res: Response): Promise<void> {
  try {
    ...
    for (const message of messages) {
      ...
      if (message.data && message.data.length > 0) {
        try {
          ...
          messageInfo.parseError = typeof e === 'object' && e !== null && 'message' in e ? (e as { message: string }).message : String(e);
        }
      }
...
async getAllBatchDetails(req: Request, res: Response): Promise<void> {
  try {
    ...
    const batchDetailsMap = new Map<string, any>();
          ...
          if (data && data.length > 0 && data[0].value) {
            ...
            if (messageData.from === 'icbt-campus' || 
              ...
              if (batchInfo && batchInfo.students && batchInfo.students.length > 0) {
                ...
                const studentsWithDetails = batchInfo.students.map((student: any) => ({
          ...
          if (data && data.length > 0 && data[0].value) {
            ...
            if (messageData.type === 'CERTIFICATE_PDFS_ISSUED' && 
              ...
              if (existingBatch) {
                ...
                const updatedStudents = existingBatch.students.map((student: any) => {
                  const certificateRef = messageData.certificate_pdf_refs?.find(
                    (cert: any) => cert.student_id === student.student_id
                  );
                ...
                const certificatesIssued = updatedStudents.filter((s: any) => s.certificate_issued).length;
                ...
                const studentsFromCerts = messageData.certificate_pdf_refs?.map((cert: any) => ({
                  student_id: cert.student_id,
    ...
    const overallStats = {
      ...
      averageProcessingTime: this.calculateAverageProcessingTime(
        allBatchDetails.filter(b => b.processingTime)
      ),
...
private calculateAverageProcessingTime(certificates: any[]): string {
...
private getFacultyBreakdown(batches: any[]): { [key: string]: number } {
...
private getAcademicYearBreakdown(batches: any[]): { [key: string]: number } {
  const breakdown: { [key: string]: number } = {};
